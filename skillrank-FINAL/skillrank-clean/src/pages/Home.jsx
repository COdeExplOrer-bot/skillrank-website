...
function CountdownTimer({ navigate }) {
  const [days, setDays] = useState(0)
  useEffect(() => {
    const launch = new Date('2025-09-01')
    const diff = launch - new Date()
    setDays(Math.max(0, Math.ceil(diff / (1000 * 60 * 60 * 24))))
  }, [])
  return (
    <>
      <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>🚀 Kolkata launch in</span>
      <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 20, color: 'var(--accent2)' }}>{days}</span>
      ...
      <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', cursor: 'pointer', textDecoration: 'underline' }} onClick={() => navigate('contact')}>Get early access</span>
    </>
  )
}
...
const testimonials = [
  { name: 'Priya M.', role: 'Customer, Kolkata', text: 'I found a verified electrician in 8 minutes. He had a Gold rank and 200+ jobs — I felt safe letting him in.', rating: 5 },
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 64, alignItems: 'center' }}>
            ...
            <div>
              <div className="animate-fade-up" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(232,66,10,0.2)', border: '1px solid rgba(232,66,10,0.4)', borderRadius: 100, padding: '6px 14px', marginBottom: 24 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--accent2)', letterSpacing: '0.04em' }}>NOW LAUNCHING IN KOLKATA</span>
              </div>
              ...
              <div className="animate-fade-up delay-3" style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                ...
                <button onClick={() => navigate('demo')} style={{
                  background: 'rgba(232,66,10,0.15)', border: '1px solid rgba(232,66,10,0.5)',
                  padding: '16px 32px', borderRadius: 100, fontSize: 15, fontWeight: 600,
                  color: '#FF6B35', fontFamily: 'var(--font-body)', cursor: 'pointer', transition: 'all 0.2s',
                }}
                onMouseEnter={e => e.target.style.background = 'rgba(232,66,10,0.25)'}
                onMouseLeave={e => e.target.style.background = 'rgba(232,66,10,0.15)'}
                >▶ Try Live Demo</button>
              </div>
              <div className="animate-fade-up delay-4" style={{ marginTop: 20, display: 'inline-flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: '10px 16px' }}>
                <CountdownTimer navigate={navigate} />
              </div>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>Electrician · Kolkata</div>
                  </div>
        ...
        <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center' }}>
          <AnimSection>
            ...
            <p style={{ fontSize: 17, color: 'rgba(255,255,255,0.8)', marginBottom: 36 }}>
              127 people already on the waitlist — launching in Salt Lake, New Town &amp; Rajarhat.
            </p>
